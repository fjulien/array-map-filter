...
function filterOffensiveComments(comments, bannedWords) {
  return (comments.filter(comment => !comment.toLowerCase().split(' ').includes(bannedWords[0].toLowerCase()) && !comment.toLowerCase().split(' ').includes(bannedWords[1].toLowerCase())))
}
